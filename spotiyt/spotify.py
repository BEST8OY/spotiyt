...
def get_token(personalized: bool = True) -> str:
    try:
        res = requests.get("https://api.github.com/gists/22ed9c6ba463899e933427f7de1f0eef", timeout=10)
        nuances = json.loads(res.json()["files"]["nuances.json"]["content"])
        nuances.sort(key=lambda x: x["v"], reverse=True)
        nuance = nuances[0]

        server_time = requests.get("https://open.spotify.com/api/server-time", timeout=10).json()["serverTime"]
    ...
    with create_progress() as progress:
        ...
        while True:
            for attempt in range(5):
                ...
                raise Exception("Max retries exceeded while fetching Spotify playlist")

            data = res.json()
            if "data" not in data or "playlistV2" not in data["data"]:
                raise Exception(f"Invalid playlist response: {data}")

            pv = data["data"]["playlistV2"]
            if offset == 0:
