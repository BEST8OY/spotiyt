...
def _split_artists(s: str) -> list[tuple[str, str]]:
    ...
    names = re.split(
        r"\s*[;,/\\|]\s+|\s+&\s+|\s+and\s+|\s+feat\.?\s+|\s+ft\.?\s+|\s+featuring\s+|\s+with\s+|\s+w/\s+|\s+[xX]\s+|\s+vs\.?\s+|\s+pres\.?\s+|\s+presents\s+",
        s_norm,
        flags=re.IGNORECASE,
    )
    result = []
...
def _artist_matches(a1_tuple: tuple[str, str], a2_tuple: tuple[str, str]) -> bool:
    a1, a1_nt = a1_tuple
    a2, a2_nt = a2_tuple
    if a1 in (a2, a2_nt) or a1_nt in (a2_nt, a2):
        return True
    if word_ratio(a1_nt, a2_nt) >= 0.75 or word_ratio(a1, a2) >= 0.75:
        return True
    if len(a1_nt) >= 3 and len(a2_nt) >= 3 and (a1_nt in a2_nt or a2_nt in a1_nt):
        return True
    flat1 = a1_nt.replace(" ", "")
    flat2 = a2_nt.replace(" ", "")
    return bool(flat1 and flat2 and SequenceMatcher(None, flat1, flat2).ratio() >= 0.8)
