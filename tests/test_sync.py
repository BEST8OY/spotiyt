...
class TestSyncLogic(unittest.TestCase):
    def test_find_unmatched(self):
        ...
        self.assertEqual(matched_ids, ["vid_a"])
        self.assertEqual(len(unmatched_spotify), 2)
        self.assertEqual([t["name"] for t in unmatched_spotify], ["Song B", "Song C"])
        self.assertEqual(len(unmatched_yt), 1)
        self.assertEqual(unmatched_yt[0]["videoId"], "vid_old")
    ...
    def test_sync_prints_added_tracks(
        ...
        mock_fetch_pl.return_value = (
            ...
            [
                ...
                {
                    "itemV2": {
                        "data": {"name": "Existing Song", "artists": {"items": [{"profile": {"name": "Artist 2"}}]}}
                    }
                },
            ],
        )
        ...
        # Verify that console.print was called with the formatted added track
        printed_texts = [call.args[0] for call in mock_console.print.call_args_list if call.args]
        added_track_printed = any(
            "[green]New Song 1[/green]" in text and "Artist 1" in text
            for text in printed_texts
            if isinstance(text, str)
        )
        self.assertTrue(added_track_printed, "Expected added track to be printed to console")
    ...
    def test_sync_dry_run_prints_added_and_removed(
        ...
        self.assertTrue(added_track_printed, "Expected added track in dry run preview")
        self.assertTrue(removed_track_printed, "Expected removed track in dry run preview")
    ...
    def test_sync_deduplicates_added_tracks(
        ...
        mock_fetch_pl.return_value = (
            ...
            [
                {
                    "itemV2": {
                        "data": {"name": "Duplicate Song", "artists": {"items": [{"profile": {"name": "Artist"}}]}}
                    }
                },
                {
                    "itemV2": {
                        "data": {"name": "Duplicate Song 2", "artists": {"items": [{"profile": {"name": "Artist"}}]}}
                    }
                },
            ],
        )
